...
class GlobalImportancePlotter:
    ...
    def _prepare_data_for_importance_analysis(
        ...
        available_predictors = [
            p for p in all_predictors if p in self.clean_data.columns
        ]
        ...
        for col in self.feature_categories + ["scale"]:
            if col in X.columns and X[col].dtype != bool and X[col].dtype != int:
                try:
                    ...
                    warnings.warn(
                        f"Could not convert column '{col}' to int. It will be treated as categorical.",
                        stacklevel=2,
                    )

        # Identify column types for the preprocessor
        numeric_features = [
            p for p in self.pipeline_continuous_params if p in X.columns
        ]

        categorical_features = [
            p
            for p in self.pipeline_categorical_params
            if p in X.columns and p != "scale"
        ]
        ...
        binary_features = [
            p
            for p in self.feature_categories
            if p in X.columns and p not in categorical_features
        ]
    ...
    def plot_global_importance(
        ...
        """
        self.logger.info(
            f"Running Global Importance Analysis for metric: {metric.upper()}"
        )
        try:
            X, y, preprocessor, numeric_features, categorical_features = (
                self._prepare_data_for_importance_analysis(metric)
            )
        except ValueError as e:
            self.logger.error(
                f"Could not prepare data for global importance analysis: {e}"
            )
            return
        ...
        ax.set_title(
            f"Global Parameter Importance for Predicting {metric.upper()}",
            fontsize=16,
            fontweight="bold",
        )
