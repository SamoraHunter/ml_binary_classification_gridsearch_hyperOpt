...
class PipelineParameterPlotter:
    ...
    def __init__(self, data: pd.DataFrame):
        ...
        self.available_categorical = [
            p for p in self.categorical_params if p in self.clean_data.columns
        ]
        self.available_continuous = {
            k: v
            for k, v in self.continuous_params.items()
            if k in self.clean_data.columns
        }

        if not self.available_categorical and not self.available_continuous:
            raise ValueError(
                "No pipeline parameter columns (e.g., 'resample', 'scale') found in the provided data."
            )
    ...
    def plot_categorical_parameters(
        ...
        for i, param in enumerate(self.available_categorical):
            ...
            if param == "scale":
                plot_data[param] = (
                    plot_data[param]
                    .astype(str)
                    .map({"True": "Scaled", "False": "Not Scaled", "None": "None"})
                )
            ...
            ax.set_title(
                f'{param.replace("_", " ").title()}', fontsize=11, fontweight="bold"
            )
        ...
        plt.suptitle(
            f"Impact of Categorical Pipeline Parameters on {metric.upper()}",
            fontsize=16,
            fontweight="bold",
        )
    ...
    def plot_continuous_parameters(
        ...
        plt.suptitle(
            f"Correlation of Continuous Pipeline Parameters with {metric.upper()}",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
