...
class ResultsSummarizer:
    ...
    def get_best_model_per_outcome(self, metric: str = 'auc') -> pd.DataFrame:
        ...
        if 'decoded_features' not in self.clean_data.columns:
            raise ValueError("Data must contain a 'decoded_features' column. "
        ...
        feature_lists = best_models_df['decoded_features'].apply(lambda x: x if isinstance(x, list) else [])

        if feature_lists.empty or feature_lists.apply(len).sum() == 0:
            warnings.warn("No features found in 'decoded_features' for the best models. Returning summary without feature columns.", stacklevel=2)
        ...
        result_df = result_df.drop(columns=['f_list', 'decoded_features'], errors='ignore')
