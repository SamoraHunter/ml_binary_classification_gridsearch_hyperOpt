...
class TimelineAnalysisPlotter:
    ...
    def _plot_single_timeline(
        ...
        if aggregation == 'mean':
            timeline_data = plot_data.groupby(['run_timestamp', 'method_name'])[metric].mean()
        elif aggregation == 'best':
            timeline_data = plot_data.groupby(['run_timestamp', 'method_name'])[metric].max()
        elif aggregation == 'median':
            timeline_data = plot_data.groupby(['run_timestamp', 'method_name'])[metric].median()
        else:
    ...
    def _plot_stratified_timeline(
        ...
        fig, axes = plt.subplots(n_outcomes, 1, figsize=(figsize[0], figsize[1] * n_outcomes / 2))
        ...
        for i, outcome in enumerate(outcomes):
            ...
            if len(outcome_data) > 0:
                # Aggregate by run and algorithm
                if aggregation == 'mean':
                    timeline_data = outcome_data.groupby(['run_timestamp', 'method_name'])[metric].mean()
                elif aggregation == 'best':
                    timeline_data = outcome_data.groupby(['run_timestamp', 'method_name'])[metric].max()
                elif aggregation == 'median':
                    timeline_data = outcome_data.groupby(['run_timestamp', 'method_name'])[metric].median()
                ...
                for algo in timeline_df.columns:
                    ...
                    if len(algo_data) > 0:
                        x_positions = range(len(algo_data))
                        ax.plot(x_positions, algo_data, marker='o', linewidth=2, 
                               markersize=4, label=algo)
                
                ax.set_title(f'{outcome} - {aggregation.title()} {metric.upper()}', 
                ...
                if i == len(outcomes) - 1:  # Only on bottom plot
                    run_timestamps = timeline_df.index.tolist()
                    tick_positions = range(0, len(run_timestamps), max(1, len(run_timestamps) // 8))
                    tick_labels = [run_timestamps[j][:10] + '...' for j in tick_positions]
        ...
        plt.suptitle(f'{aggregation.title()} {metric.upper()} Timeline by Outcome', 
    ...
    def _plot_single_intra_run_progress(
        ...
        for run_id in runs:
            ...
            plt.plot(cummax_perf.index, cummax_perf, label=f'Run {run_id[:10]}...', alpha=0.8)

        plt.xlabel('Trial Number (within run)', fontsize=12)
        plt.ylabel(f'Best {metric.upper()} So Far', fontsize=12)
        plt.title(f'Optimization Progress per Run - All Outcomes', fontsize=14, fontweight='bold')
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.show()
    ...
    def _plot_stratified_intra_run_progress(
        ...
        for i, outcome in enumerate(outcomes):
            ...
            for run_id in runs:
                ...
                ax.plot(cummax_perf.index, cummax_perf, label=f'Run {run_id[:10]}...', alpha=0.7)
        ...
        plt.suptitle(f'Optimization Progress per Run by Outcome', fontsize=16, fontweight='bold')
