...
class FeatureCategoryPlotter:
    ...
    def plot_category_performance_boxplots(
        ...
        for i, category in enumerate(self.available_categories):
            ...
            if plot_data[category].dtype != bool:
                try:
                    ...
                    warnings.warn(f"Could not convert column '{category}' to boolean. Skipping.", stacklevel=2)
                    ax.text(0.5, 0.5, 'Invalid Data Type', ha='center', va='center', transform=ax.transAxes)
                    ax.set_title(category, fontsize=11)
                    continue

            sns.boxplot(data=plot_data, x=category, y=metric, ax=ax, palette="Set2", hue=category, legend=False)
            ax.set_title(f'{category.replace("_", " ").title()}', fontsize=11, fontweight='bold')
        ...
        plt.suptitle(f'Impact of Including Feature Categories on {metric.upper()}', fontsize=16, fontweight='bold')
