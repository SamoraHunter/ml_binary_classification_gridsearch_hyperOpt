...
class HyperparameterAnalysisPlotter:
    ...
    def __init__(self, data: pd.DataFrame):
        if "algorithm_implementation" not in data.columns:
            raise ValueError(
                "Data must contain an 'algorithm_implementation' column for hyperparameter analysis."
            )
        ...
        self.clean_data["algorithm_name"] = self.clean_data[
            "algorithm_implementation"
        ].apply(
            lambda x: (
                x.split("(")[0].strip() if isinstance(x, str) and "(" in x else None
            )
        )

        # Filter out ignored algorithms before parsing
        self.clean_data = self.clean_data[
            ~self.clean_data["algorithm_name"].isin(self._ALGOS_TO_IGNORE)
        ]

        # Parse parameters
        self.clean_data["params_dict"] = self.clean_data[
            "algorithm_implementation"
        ].apply(self._parse_model_string_to_params)

        # Drop rows where parsing failed
        self.clean_data = self.clean_data.dropna(
            subset=["params_dict", "algorithm_name"]
        ).copy()
    ...
    def _parse_model_string_to_params(model_str: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            warnings.warn(
                f"Failed to parse model string '{model_str[:50]}...'. Error: {e}"
            )
            return None
    ...
    def plot_performance_by_hyperparameter(
        ...
        """
        algo_data = self.clean_data[
            self.clean_data["algorithm_name"] == algorithm_name
        ].copy()
        ...
        if n_params == 0:
            self.logger.info(
                f"No valid hyperparameters were provided to plot for algorithm '{algorithm_name}'."
            )
            return
        ...
        for i, param in enumerate(hyperparameters):
            ax = axes[i]
            self._plot_single_performance_vs_hyperparameter(
                ax, algo_data, param, metric
            )
        ...
        fig.suptitle(
            f"Performance vs. Hyperparameters for {algorithm_name}",
            fontsize=16,
            fontweight="bold",
        )
    ...
    def _plot_single_performance_vs_hyperparameter(
        self, ax: plt.Axes, algo_data: pd.DataFrame, hyperparameter: str, metric: str
    ):
        ...
        plot_data[hyperparameter] = plot_data["params_dict"].apply(
            lambda p: p.get(hyperparameter) if p else None
        )
        ...
        if is_numeric and (is_float or param_values.nunique() > 8):
            ...
            if (
                "outcome_variable" in plot_data.columns
                and plot_data["outcome_variable"].nunique() > 1
            ):
            ...
            sns.scatterplot(
                data=plot_data, x=hyperparameter, y=metric, ax=ax, **scatter_kwargs
            )
            ...
            ax.set_title(
                f"{metric.upper()} vs. {hyperparameter}", fontsize=11, fontweight="bold"
            )
            ...
            if is_numeric:
                ...
                sns.boxplot(
                    data=plot_data, x=hyperparameter, y=metric, order=order, ax=ax
                )
            else:
            ...
            ax.set_title(
                f"{metric.upper()} by {hyperparameter}", fontsize=11, fontweight="bold"
            )

        ax.set_ylabel(metric.upper(), fontsize=10)
        ax.grid(True, alpha=0.3)

    def _get_continuous_hyperparameter_correlations(
        self, algorithm_name: str, metric: str, method: str = "pearson"
        ...
        for param in sorted(list(all_params)):
            ...
            if (
                not param_values.empty
                and is_numeric
                and (is_float or param_values.nunique() > 8)
            ):
    ...
    def plot_hyperparameter_importance(
        ...
        if metric not in algo_data.columns:
            self.logger.error(
                f"Metric '{metric}' not found. Available metrics: {algo_data.select_dtypes(include=[np.number]).columns.tolist()}"
            )
            return
        ...
        if top_models.empty:
            self.logger.info(
                f"No models found in the top {top_n_percent}% for algorithm '{algorithm_name}'."
            )
            return
        ...
        for i, param in enumerate(hyperparameters):
            ...
            if is_numeric and (is_float or all_values.nunique() > 8):
                ...
                plot_df[param] = plot_df["params_dict"].apply(
                    lambda p: p.get(param) if p else None
                )
                ...
                ax.set_title(
                    f"{metric.upper()} vs. {param}", fontsize=11, fontweight="bold"
                )
                ax.set_xlabel(param)
                ...
                if (
                    not plot_df.empty
                    and plot_df[param].min() > 0
                    and plot_df[param].max() / plot_df[param].min() > 100
                ):
                    ax.set_xscale("log")
                ...
                if leg:
                    ...
                    for t in leg.get_texts():
                        if t.get_text() == "False":
                            t.set_text(f"Bottom {100-top_n_percent}%")
                        if t.get_text() == "True":
                            t.set_text(f"Top {top_n_percent}%")
            # Case 2: Discrete numeric or Categorical data
                ...
                if is_numeric:
                    ...
                    df_plot = (
                        pd.concat(
                            [all_counts.rename("All"), top_counts.rename("Top")], axis=1
                        )
                        .fillna(0)
                        .sort_index()
                    )
                    ...
                    ax.get_xaxis().set_major_formatter(
                        plt.FuncFormatter(lambda x, p: f"{x:g}")
                    )  # Use general format for ticks
                    plt.setp(
                        ax.get_xticklabels(), rotation=45, ha="right"
                    )  # Correctly set rotation and alignment
        ...
        plt.suptitle(
            f"Hyperparameter Analysis: {algorithm_name}\n(All vs. Top {top_n_percent}% by {metric.upper()})",
            fontsize=16,
            fontweight="bold",
        )
    ...
    def plot_hyperparameter_correlations(
        self,
        algorithm_name: str,
        metric: str = "auc",
        method: str = "pearson",
        ...
        # Get correlations
        correlation_results_df = self._get_continuous_hyperparameter_correlations(
            algorithm_name, metric, method
        )

        if correlation_results_df is None or correlation_results_df.empty:
            self.logger.info(
                f"No continuous hyperparameters found for {algorithm_name}."
            )
            return
        ...
        plt.suptitle(
            f"Hyperparameter Correlations with {metric.upper()}: {algorithm_name}",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        ...
        if show_correlation_stats:
            ...
            self.logger.info("-" * 60)
            corr_df = correlation_results_df.sort_values(
            ...
            for _, row in corr_df.iterrows():
                ...
                self.logger.info(
                    f"{row['hyperparameter']:20s}: {corr_label} = {row['correlation']:6.3f}{significance:3s} "
    ...
    def plot_top_correlations(
        ...
        correlations_df = self._get_continuous_hyperparameter_correlations(
            algorithm_name, metric, method
        )

        if correlations_df is None or correlations_df.empty:
            self.logger.info(
                f"No continuous hyperparameters found for {algorithm_name} to plot correlations."
            )
            return
        top_correlations = correlations_df.sort_values(
            "abs_correlation", ascending=False
        ).head(top_n)
        ...
        plt.suptitle(
            f"Top {min(top_n, len(top_correlations))} Hyperparameter Correlations: {algorithm_name}",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout(rect=[0, 0, 1, 0.96])
