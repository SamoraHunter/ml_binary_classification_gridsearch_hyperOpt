...
try:
    ...
    UpSet = None
    warnings.warn(
        "`upsetplot` library not found. `plot_feature_set_intersections` will be unavailable. "
...
class FeatureAnalysisPlotter:
    ...
    def __init__(self, data: pd.DataFrame):
        ...
        if "decoded_features" not in data.columns:
            raise ValueError(
                "Data must contain a 'decoded_features' column. Ensure ResultsAggregator was run with a feature names CSV."
            )
        ...
        self.feature_df = (
            self.clean_data.explode("decoded_features")
            .rename(columns={"decoded_features": "feature"})
    ...
    def plot_feature_usage_frequency(
        self,
        top_n: int = 20,
        ...
        if not stratify_by_outcome:
            ...
            if "outcome_variable" not in self.clean_data.columns:
                raise ValueError(
                    "outcome_variable column not found for stratification."
                )
    ...
    def _plot_single_feature_frequency(self, top_n: int, figsize: Tuple[int, int]):
        ...
        plt.title(
            f"Top {top_n} Most Frequently Used Features (All Outcomes)",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def _plot_stratified_feature_frequency(
        self,
        top_n: int,
        ...
        for i, outcome in enumerate(outcomes):
            ax = axes[i]
            outcome_feature_df = self.feature_df[
                self.feature_df["outcome_variable"] == outcome
            ]

            if not outcome_feature_df.empty:
                feature_counts = (
                    outcome_feature_df["feature"].value_counts().nlargest(top_n)
                )
                ...
                if not feature_counts.empty:
                    ...
                    ax.set_title(
                        f"{outcome} - Top {min(top_n, len(feature_counts))} Features",
                        fontsize=11,
                        fontweight="bold",
                    )
        ...
        plt.suptitle(
            f"Top {top_n} Most Frequently Used Features per Outcome",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout()
    ...
    def plot_feature_performance_impact(
        self,
        metric: str = "auc",
        ...
        outcomes_to_plot = outcomes or sorted(
            self.clean_data["outcome_variable"].unique()
        )

        if (
            len(outcomes_to_plot) > MAX_OUTCOMES_FOR_STRATIFIED_PLOT
        ...
        for outcome in outcomes_to_plot:
            ...
            if outcome_data.empty:
                warnings.warn(
                    f"Skipping plot for outcome '{outcome}': No successful data available.",
                    stacklevel=2,
                )
                continue
            ...
            non_empty_decoded_features_count = (
                outcome_data["decoded_features"]
            ...
            if len(all_features_in_outcome) > top_n_features_to_consider:
                ...
                features_to_analyze = feature_counts.nlargest(
                    top_n_features_to_consider
                ).index.tolist()

            impact_data = []
            for feature in features_to_analyze:
                # Create a boolean mask for runs containing the feature
                has_feature_mask = outcome_data["decoded_features"].apply(
                ...
                mean_without_feature = outcome_data.loc[
                    ~has_feature_mask, metric
                ].mean()
            ...
            if not impact_data:  # Final check after iterating through all features
                warnings.warn(
                    f"Skipping plot for outcome '{outcome}': No features met the analysis criteria "
                    f"after filtering (min_usage={min_usage}, always present/absent).",
                    stacklevel=2,
                )
                continue
            ...
            plot_df = (
                pd.concat([top_impact, bottom_impact])
                .drop_duplicates()
            ...
            ax.set_title(
                f"Feature Impact on {metric.upper()} for Outcome: {outcome}",
                fontsize=14,
                fontweight="bold",
            )
            ax.set_xlabel(
                f"Change in Mean {metric.upper()} (With vs. Without Feature)",
                fontsize=12,
            )
    ...
    def plot_feature_metric_correlation(
        self,
        metric: str = "auc",
        outcomes: Optional[List[str]] = None,
        top_n: int = 20,
        ...
        for outcome in outcomes_to_plot:
            ...
            if not correlation_data:
                warnings.warn(
                    f"Skipping plot for outcome '{outcome}': No features met the analysis criteria (min_usage={min_usage}).",
                    stacklevel=2,
                )
                continue
            ...
            plot_df = (
                pd.concat([top_corr, bottom_corr])
                .drop_duplicates()
            ...
            ax.set_title(
                f"Feature Correlation with {metric.upper()} for Outcome: {outcome}",
                fontsize=14,
                fontweight="bold",
            )
            ax.set_xlabel(
                f"Point-Biserial Correlation with {metric.upper()}", fontsize=12
            )
            ax.set_ylabel("Feature", fontsize=12)
            ax.axvline(0, color="black", linewidth=0.8, linestyle="--")
    ...
    def plot_feature_set_intersections(
        self,
        top_n_sets: int = 10,
        ...
        if UpSet is None:
            warnings.warn(
                "Cannot generate UpSet plot because `upsetplot` is not installed. "
                "Install with: pip install upsetplot",
                stacklevel=2,
            )
            return
    ...
    def _plot_single_upset(
        self,
        data: pd.DataFrame,
        title: str,
        top_n_sets: int,
        min_subset_size: int,
        max_features_for_upset: int,
        figsize: Tuple[int, int],
    ):
        """A helper method to generate a single UpSet plot for given data."""
        # Filter out rows with empty or invalid feature lists
        feature_sets = (
            data["decoded_features"]
            .dropna()
            .apply(lambda x: x if isinstance(x, list) and x else None)
            .dropna()
        )

        if feature_sets.empty:
            warnings.warn(
                f"No valid feature sets to plot for '{title}'. Skipping.", stacklevel=3
            )
            return
        ...
        all_features_flat = [
            feature for feature_list in feature_sets for feature in feature_list
        ]
        if not all_features_flat:
            warnings.warn(
                f"No features found in feature sets for '{title}'. Skipping.",
                stacklevel=3,
            )
            return
        ...
        if len(feature_counts) > max_features_for_upset:
            ...
            feature_sets = feature_sets.apply(
                lambda x: [f for f in x if f in top_features]
            )
        ...
        if feature_sets.empty:
            warnings.warn(
                f"No feature sets remaining for '{title}' after filtering for top features. Skipping.",
                stacklevel=3,
            )
            return
        ...
        if upset_source_data.empty:
            warnings.warn(
                f"Could not generate upset data from feature sets for '{title}'. Skipping.",
                stacklevel=3,
            )
            return

        max_intersection_size = (
            upset_source_data.max() if not upset_source_data.empty else 0
        )
        ...
        upset.plot(fig=fig)
        plt.suptitle(
            f"Feature Set Intersections - {title}", fontsize=16, fontweight="bold"
        )
        plt.show()
