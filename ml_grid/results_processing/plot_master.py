...
class MasterPlotter:
    ...
    def __init__(self, data: pd.DataFrame, output_dir: str = "."):
        ...
        try:
            ...
            self.logger.warning(
                f"Could not initialize GlobalImportancePlotter. Reason: {e}"
            )
        ...
        try:
            ...
            self.logger.warning(
                f"Could not initialize PipelineParameterPlotter. Reason: {e}"
            )
        ...
        try:
            ...
            self.logger.warning(
                f"Could not initialize FeatureCategoryPlotter. Reason: {e}"
            )
        ...
        if "decoded_features" in self.data.columns:
            ...
            self.logger.warning(
                "'decoded_features' column not found. Feature-related plots will be skipped."
            )
        ...
        if "algorithm_implementation" in self.data.columns:
            ...
            self.logger.warning(
                "'algorithm_implementation' column not found. Hyperparameter-related plots will be skipped."
            )
        ...
        try:
            ...
            self.logger.warning(
                f"Could not initialize BestModelAnalyzerPlotter. Reason: {e}"
            )
    ...
    def plot_all(
        ...
        if self.summarizer and save_best_results:
            ...
            try:
                ...
                self.logger.info(
                    f"   - Using metric '{summary_metric}' for summary table."
                )

                if "outcome_variable" not in self.data.columns:
                    raise ValueError(
                        "'outcome_variable' column is required to find the best model per outcome."
                    )  # This will be caught and logged
                if "decoded_features" not in self.data.columns:
                    raise ValueError(
                        "'decoded_features' column is required to expand feature names."
                    )

                best_models_df = self.summarizer.get_best_model_per_outcome(
                ...
                self.logger.info(
                    f"✅ Best models summary successfully saved to: {os.path.abspath(output_path)}"
                )

            except Exception as e:
                self.logger.error(
                    f"❌ Could not generate or save best models summary table. Reason: {e}"
                )
        ...
        try:
            self.algo_plotter.plot_algorithm_boxplots(
                metric=metric, stratify_by_outcome=stratify_by_outcome
            )
            self.algo_plotter.plot_algorithm_performance_heatmap(
                metric=metric, aggregation="mean"
            )
            self.algo_plotter.plot_algorithm_ranking(
                metric=metric,
                stratify_by_outcome=stratify_by_outcome,
                top_n=top_n_algorithms,
            )
            self.algo_plotter.plot_algorithm_stability(
                metric=metric, top_n=top_n_algorithms
            )
            self.algo_plotter.plot_performance_tradeoff(
                metric_y=metric, metric_x="run_time", top_n_algos=top_n_algorithms
            )
        ...
        try:
            self.timeline_plotter.plot_performance_timeline(
                metric=metric, stratify_by_outcome=stratify_by_outcome
            )
            self.timeline_plotter.plot_improvement_trends(
                metric=metric, stratify_by_outcome=stratify_by_outcome
            )
            self.timeline_plotter.plot_computational_cost_timeline(
                stratify_by_outcome=stratify_by_outcome
            )
        except Exception as e:
        ...
        if self.feature_plotter:
            ...
            try:
                self.feature_plotter.plot_feature_usage_frequency(
                    top_n=top_n_features, stratify_by_outcome=stratify_by_outcome
                )
                self.feature_plotter.plot_feature_performance_impact(
                    metric=metric, top_n=top_n_features // 2
                )
                self.feature_plotter.plot_feature_metric_correlation(
                    metric=metric, top_n=top_n_features // 2
                )
                self.feature_plotter.plot_feature_set_intersections(
                    top_n_sets=15, stratify_by_outcome=stratify_by_outcome
                )
            except Exception as e:
