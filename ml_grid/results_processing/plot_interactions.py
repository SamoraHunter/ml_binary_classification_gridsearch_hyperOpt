...
class InteractionPlotter:
    ...
    def plot_categorical_interaction(
        ...
        """
        if (
            param1 not in self.clean_data.columns
            or param2 not in self.clean_data.columns
        ):
            self.logger.warning(
                f"One or both parameters ('{param1}', '{param2}') not found. Skipping interaction plot."
            )
            return
        ...
        try:
            ...
            self.logger.error(
                f"Could not create pivot table for '{param1}' and '{param2}'. Reason: {e}"
            )
            return
    ...
    def plot_continuous_interaction(
        ...
        if plot_data.empty:
            self.logger.info(
                f"No data to plot for interaction between '{param1}' and '{param2}'."
            )
            return
        ...
        plt.title(
            f"Interaction of {param1.title()} and {param2.title()} on {metric.upper()}",
            fontsize=14,
            fontweight="bold",
        )
