...
class ResultsAggregator:
    """
    Core class to aggregate and manage ML optimization results from hierarchical folder structure.
    
    Structure: root_folder/timestamp_folder/final_grid_score_log.csv
    """
    
    def __init__(self, root_folder: str, feature_names_csv: Optional[str] = None):
        """
        Initialize the aggregator.
        
        Args:
            root_folder: Path to the master root folder
            feature_names_csv: Path to CSV containing original feature names mapping
        """
    ...
    def load_feature_names(self, feature_names_csv: str):
        """
        Load feature names from a CSV file, assuming the feature names are the
        ...
        try:
            ...
            warnings.warn(f"Could not load feature names from {feature_names_csv}: {e}", stacklevel=2)
    
    def get_available_runs(self) -> List[str]:
        """Get list of available timestamp folders."""
    ...
    def load_single_run(self, timestamp_folder: str) -> pd.DataFrame:
        """Load results from a specific timestamp folder."""
    ...
    def aggregate_all_runs(self) -> pd.DataFrame:
        """Aggregate results from all available runs."""
    ...
    def aggregate_specific_runs(self, run_names: List[str]) -> pd.DataFrame:
        """Aggregate results from specific runs only."""
        all_dataframes = []
    ...
    def _decode_features(self, feature_string: str) -> List[str]:
        """
        Decodes a feature list string into a list of active feature names by following a three-step process:
        1. Parse the string representation (e.g., "[[0, 1, 0]]") into a simple binary list.
        2. Validate that the length of the binary list matches the master list of feature names.
        3. Map the binary list to the feature names, selecting names where the flag is 1.

        Args:
            feature_string: String representation of a feature list, like "[[0, 1, 0, 0, 1]]".

        Returns:
            A list of feature names that were selected (where the value is 1).
        """
        if self.feature_names is None:
            warnings.warn("Cannot decode features: feature_names are not loaded. Please ensure a feature_names_csv was provided during ResultsAggregator initialization.", stacklevel=2)
            return []
        ...
        try:
            ...
            # Make parsing robust by extracting list content from formats like 'array([[0, 1]])'
            ...
            while isinstance(binary_map, (list, tuple)) and len(binary_map) > 0 and isinstance(binary_map[0], (list, tuple)):
                binary_map = binary_map[0]

            if not binary_map:
                return []

            # Step 2: Confirm the length of the binary list matches the original feature names.
            if len(binary_map) != len(self.feature_names):
                warnings.warn(
                    f"Feature list length mismatch for string '{feature_string}'. "
                    f"Parsed list has length {len(binary_map)}, but loaded feature names have length {len(self.feature_names)}. "
                    "This will lead to incorrect feature mapping. Returning empty list. "
                    "Please ensure 'feature_names_csv' corresponds to the feature set used during the experiment run.",
                    stacklevel=2
                )
                return []

            # Step 3: Use the map to decode the feature names where the flag is 1.
            # int(flag) handles both integer (1) and float (1.0) representations.
            return [name for name, flag in zip(self.feature_names, binary_map) if int(flag) == 1]

        except (ValueError, SyntaxError, TypeError, IndexError) as e:
            warnings.warn(f"Could not decode feature string: '{feature_string}'. It may be malformed or contain non-numeric values. Error: {e}", stacklevel=2)
            return []
    
    def get_summary_stats(self, data: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        """Get summary statistics for the aggregated results."""
        if data is None:
    ...
    def get_outcome_variables(self, data: Optional[pd.DataFrame] = None) -> List[str]:
        """Get list of unique outcome variables in the data."""
        if data is None:
            data = self.aggregated_data
    ...
    def get_data_by_outcome(self, outcome_variable: str, data: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        """Get data filtered by specific outcome variable."""
        if data is None:
            data = self.aggregated_data
    ...
    def get_outcome_summary(self, data: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        """Get summary statistics stratified by outcome variable."""
        if data is None:
            data = self.aggregated_data
        ...
        outcome_summary = clean_data.groupby('outcome_variable')[available_metrics].agg([
...
class DataValidator:
    """Utility class for validating and checking data quality."""
    
    @staticmethod
    def validate_data_structure(df: pd.DataFrame) -> Dict[str, any]:
        """Validate the structure and quality of results data."""
        validation_report = {
        ...
        expected_columns = ['method_name', 'auc', 'f1', 'precision', 'recall', 'accuracy']
        missing_columns = [col for col in expected_columns if col not in df.columns]
        validation_report['missing_columns'] = missing_columns
        
        # Check for outcome variables
        if 'outcome_variable' in df.columns:
            validation_report['outcome_variables'] = df['outcome_variable'].unique().tolist()
    ...
    @staticmethod
    def print_validation_report(validation_report: Dict[str, any]):
...
def get_clean_data(df: pd.DataFrame, remove_failed: bool = True) -> pd.DataFrame:
    """Utility function to get clean data for analysis."""
    if remove_failed and 'failed' in df.columns:
        return df[df['failed'] == 0].copy()
    return df.copy()


def stratify_by_outcome(df: pd.DataFrame, func, *args, **kwargs) -> Dict[str, any]:
    """
    Utility function to apply a function to data stratified by outcome variable.
    
    Args:
        df: DataFrame with outcome_variable column
        func: Function to apply to each outcome's data
        *args, **kwargs: Arguments to pass to the function
    
    Returns:
        Dictionary with outcome variables as keys and function results as values
    """
