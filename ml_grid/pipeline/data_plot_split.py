...
def plot_pie_chart_with_counts(X_train, X_test, X_test_orig):
    ...
    explode = tuple(0.1 if i == sizes.index(max(sizes)) else 0 for i in range(len(sizes)))

    # Create the pie chart
    plt.figure(figsize=(2, 2))
    patches, texts, autotexts = plt.pie(sizes, labels=labels, colors=colors, autopct='%1.1f%%', startangle=140, explode=explode)
...
def plot_dict_values(data_dict):
    ...
    legend_colors = [plt.Rectangle((0, 0), 1, 1, fc='green', edgecolor='none'),
...
def create_bar_chart(data_dict, title='', x_label='', y_label=''):
    ...
    for index, value in enumerate(values):
        plt.text(value, index, str(value), ha='left', va='center', fontsize=10, fontweight='bold', color='black')
...
def plot_candidate_feature_category_lists(data):
    ...
    create_bar_chart(data, title='Feature category counts', x_label='features', y_label='counts')
