...
class MockH2OEstimator:
    ...
    def __init__(
        self, export_checkpoints_dir=None, **kwargs
...
def test_fit_successful(
    mock_h2o_init, mock_h2o_cluster, mock_h2o_frame, classifier_instance, sample_data
):
    """
    ...
    # We expect one call to H2OFrame with a pandas DataFrame that has X and y
    # concatenated
    pd.testing.assert_frame_equal(
        mock_h2o_frame.call_args[0][0].drop("outcome", axis=1), X
    )
    pd.testing.assert_series_equal(
        mock_h2o_frame.call_args[0][0]["outcome"].reset_index(drop=True),
        y.astype("category").reset_index(drop=True),
        check_names=False,
    )
    ...
    # We can't directly check the call to train because the model object is
    # created inside `fit`, but we can verify the side-effects.
    ...
    assert classifier_instance.feature_types_ == {
        "feature1": "real",
        "feature2": "real",
        "feature3": "enum",
    }
...
def test_predict_successful(
    ...
    classifier_instance.feature_types_ = {
        "feature1": "real",
        "feature2": "real",
        "feature3": "enum",
    }
...
def test_predict_on_unfitted_model_raises_error(classifier_instance, sample_data):
    ...
    X, _ = sample_data
    with pytest.raises(
        RuntimeError, match="This H2OBaseClassifier instance is not fitted yet"
    ):
...
@patch("h2o.cluster")
@patch("h2o.init")
def test_fit_with_checkpointing(
    mock_h2o_init, mock_h2o_cluster, mock_h2o_frame, sample_data
):
    """
