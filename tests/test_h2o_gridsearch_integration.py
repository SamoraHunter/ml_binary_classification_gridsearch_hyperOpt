...
def test_h2o_search_integrations(
    model_class, search_strategy, synthetic_data, h2o_session_fixture
):
    """
    ...
    param_space = _prepare_h2o_param_space(
        instance=instance, model_class=model_class, search_strategy=search_strategy
    )
...
)  # Only test one strategy for slow models
@pytest.mark.parametrize("model_class", H2O_SLOW_MODELS)
def test_h2o_slow_models(
    model_class, search_strategy, synthetic_data, h2o_session_fixture
):
    """
