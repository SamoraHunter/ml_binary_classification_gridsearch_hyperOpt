...
class TestRemoveConstantColumns(unittest.TestCase):
    ...
    def test_remove_constant_columns_with_constants(self):
        ...
        df = pd.DataFrame(
            {"a": [1, 2, 3], "b": [5, 5, 5], "c": ["x", "y", "z"], "d": [0, 0, 0]}
        )
        initial_drop_list = ["e"]
        updated_drop_list = remove_constant_columns(
            df, initial_drop_list.copy(), verbose=0
        )
        ...
        X_train = np.array(
            [
                [
                    ...
                    [5, 5],
                    [1, 1],
                ],  # Sample 1: Feature 1 varies, Feature 2 is constant
