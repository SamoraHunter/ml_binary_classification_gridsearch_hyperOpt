...
class TestDataIntegrity(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data = {
            ...
            "also_highly_correlated": [
                x * 2.0 + np.random.normal(0, 0.001) for x in range(n_samples)
            ],
    ...
    def test_constant_in_train_removed_from_all_splits(self):
        ...
        if "constant_in_train" in pipeline.X_train.columns:
            ...
            fail_msg = (
                f"constant_in_train was not removed from X_train. "
    ...
    def test_highly_correlated_features_removed(self):
        """
        Verify that highly correlated features are correctly identified and one
        is removed.
        """
        pipeline = pipe(
            file_name=str(self.test_data_path),
        ...
        # 'highly_correlated' and 'also_highly_correlated' are designed to be
        # correlated > 0.99
        # One of them should be dropped. We check if at least one is gone.
        remaining_corr_cols = {
            "highly_correlated",
            "also_highly_correlated",
        }.intersection(pipeline.X_train.columns)

        self.assertLess(
            len(remaining_corr_cols),
            2,
